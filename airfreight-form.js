/**
 * Airfreight Registration Form Handler
...
const AIRFREIGHT_CONFIG = CONFIG?.sheets?.airfreight;

if (!AIRFREIGHT_CONFIG) {
    console.error('Airfreight Sheet configuration not found!');
}

// Form Elements
const form = document.getElementById('airfreightForm');
...
form.addEventListener('submit', async function (e) {
    ...
    const rawFormData = {
        ...
        course: 'International Air Freight Management',
        sessions: sessionsArray,
        sessionDates: datesArray,
        sheetName: AIRFREIGHT_CONFIG.sheetName || 'Airfreight'
    };
    ...
    if (FormUtils && AIRFREIGHT_CONFIG.scriptUrl) {
        try {
            setLoadingState(true);
            const isDuplicate = await FormUtils.checkEmailDuplicate(formData.email, AIRFREIGHT_CONFIG.scriptUrl, AIRFREIGHT_CONFIG.sheetName);

            if (isDuplicate) {
                setLoadingState(false);
                const proceed = confirm('อีเมลนี้เคยลงทะเบียนแล้วในหลักสูตรนี้ คุณต้องการดำเนินการต่อหรือไม่?');
                if (!proceed) {
                    return;
                }
            }
    ...
    await FormUtils.submitForm(formData, AIRFREIGHT_CONFIG.scriptUrl, {
...
if (emailInput) {
    emailInput.addEventListener('blur', async function (e) {
        ...
        if (email && FormUtils && AIRFREIGHT_CONFIG.scriptUrl) {
            ...
            emailCheckTimeout = setTimeout(async () => {
                ...
                try {
                    ...
                    const isDuplicate = await FormUtils.checkEmailDuplicate(email, AIRFREIGHT_CONFIG.scriptUrl, AIRFREIGHT_CONFIG.sheetName);

                    if (isDuplicate) {
                        e.target.setCustomValidity('อีเมลนี้เคยลงทะเบียนหลักสูตรนี้แล้ว');
                        e.target.reportValidity();
                    } else {
                        e.target.setCustomValidity('');
                    }
...
console.log('✅ Airfreight Registration Form initialized!');
console.log('📊 Sheet ID:', AIRFREIGHT_CONFIG.id);
console.log('✈️ Course: International Air Freight Management');
