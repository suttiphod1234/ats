...
const CFO_SHEET_CONFIG = CONFIG?.sheets?.cfo;

if (!CFO_SHEET_CONFIG) {
    console.error('CFO Sheet configuration not found!');
}
...
form.addEventListener('submit', async function (e) {
    ...
    const rawFormData = {
        ...
        course: 'Carbon Footprint for Organization',
        sessions: sessionsArray,
        sessionDates: datesArray,
        sheetName: CFO_SHEET_CONFIG.sheetName || 'อบรม CFO'
    };
    ...
    if (FormUtils && CFO_SHEET_CONFIG.scriptUrl) {
        try {
            setLoadingState(true);
            const isDuplicate = await FormUtils.checkEmailDuplicate(formData.email, CFO_SHEET_CONFIG.scriptUrl, CFO_SHEET_CONFIG.sheetName);
    ...
    await FormUtils.submitForm(formData, CFO_SHEET_CONFIG.scriptUrl, {
...
if (emailInput) {
    emailInput.addEventListener('blur', async function (e) {
        ...
        if (email && FormUtils && CFO_SHEET_CONFIG.scriptUrl) {
            ...
            emailCheckTimeout = setTimeout(async () => {
                ...
                try {
                    ...
                    const isDuplicate = await FormUtils.checkEmailDuplicate(email, CFO_SHEET_CONFIG.scriptUrl, CFO_SHEET_CONFIG.sheetName);
...
console.log('✅ CFO Registration Form initialized!');
console.log('📊 Sheet ID:', CFO_SHEET_CONFIG.id);
console.log('🌱 Course: Carbon Footprint for Organization');
